...
// points to the aformentioned spot in the upload heap and returns that CBV (a GPU descriptor handle).
// 
...
{
    // How much space will we need? Each CBV msut point to a chunk of the upload heap that is
    // a multiple of 256 bytes, so we need to calculate and reserve that amount.
    SIZE_T reservationSize = (SIZE_T)dataSizeInBytes;
    reservationSize = (reservationSize + 255) / 265 * 256; // Integer division trick
    ...
    {
        // Calculate the actual upload address (which we got form mapping the buffer)
...
D3D12_CPU_DESCRIPTOR_HANDLE DX12Helper::LoadTexture(const wchar_t* file, bool generateMips)
{
    ...
    // texture SRVs into the same descriptor heap, bu we don't know how many we'll need
    // until they're all loaded and thsi is a quick and dirty implementation!
    ...
    // Note: Using a null description results in the "default" SRV 
    // (same format, all mips, all array slices, etc.)
...
// alignment - What's the buffer alignment? Defautl is 0
// --------------------------------------------------------
Microsoft::WRL::ComPtr<ID3D12Resource> DX12Helper::CreateBuffer(
    UINT64 size, 
    ...
    UINT64 alignment)
{
    ...
    // Create the buffer
    device->CreateCommittedResource(&heapDesc, D3D12_HEAP_FLAG_NONE, &desc, state, 0,
...
// --------------------------------------------------------
// Gets the command allocator we can use to reset a command list
// --------------------------------------------------------
